    ...
    const unsub = onAuthStateChanged(auth, async (u) => {
      ...
      for (const mid of members) {
        ...
        if (ms.exists()) {
          ...
          if (gData.nameMode === "position") {
            names[mid] = userData.positionName || userData.displayName || "Unknown";
          } else {
            names[mid] = userData.displayName || "Unknown";
          }
        }
      }
...
console.log('Done! Chat v3 with position/nickname mode written.');
